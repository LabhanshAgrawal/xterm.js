...
import { ILinkProvider, ILink, Terminal, IViewportRange, IBufferCellPosition } from 'xterm';
...
export class LinkComputer {
  public static computeLink(y: number, regex: RegExp, terminal: Terminal, activate: (event: MouseEvent, uri: string) => void): ILink[] {
    ...
    while ((match = rex.exec(line)) !== null) {
      ...
      const range = {
        ...
        end: LinkComputer._stringIndexToBufferPosition(terminal, startLineIndex, stringIndex + text.length - 1)
      };
  ...
  private static _stringIndexToBufferPosition(terminal: Terminal, lineIndex: number, stringIndex: number): IBufferCellPosition {
